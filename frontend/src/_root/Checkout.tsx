        ...
        <div className="mt-12 lg:grid lg:grid-cols-12 lg:items-start lg:gap-x-12 xl:gap-x-16">
          ...
          >
            ...
            {isMounted && items.length === 0 && (
              <div className="flex h-full flex-col items-center justify-center space-y-1">
                <div aria-hidden="true" className="relative mb-4 h-40 w-40 text-muted-foreground">
                  <img src="/hippo-empty-cart.png" loading="eager" alt="empty shopping cart hippo" />
                </div>
            ...
            <ul className={cn({ 'divide-y divide-gray-200 border-b border-t border-gray-200': isMounted && items.length > 0 })}>
              {isMounted &&
                items.map(({ product }) => {
                  const label = PRODUCT_CATEGORIES.find((c) => c.value === product.category)?.label || 'Unknown';
                        ...
                        <div className="relative h-24 w-24">
                          {image && image.image_url && (
                            <img
                              src={image.image_url}
                              alt="product image"
                              className="h-full w-full max-w-full max-h-full rounded-md object-cover object-center sm:h-48 sm:w-48"
                            />
                          )}
                        </div>
                      </div>
                          ...
                          <div>
                            <div className="flex justify-between">
                              <h3 className="text-sm">
                                <Link to={`/product/${product.id}`} className="font-medium text-gray-500 hover:text-gray-800">
                                  {product.name}
                                </Link>
                              </h3>
                            </div>
                            ...
                            <p className="mt-1 text-sm font-medium text-gray-600">{formatPrice(product.price)}</p>
                          </div>
